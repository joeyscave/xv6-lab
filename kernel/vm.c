...
// Look up a virtual address, return the physical address,
// or 0 if not mapped.
// Can be used to look up kernel pages.
uint64
kwalkaddr(uint64 va)
{
  pte_t *pte;
...
// unmapping kernel virtual memory
void kvmunmap(pagetable_t pagetable, int level, int *c)
{
  for (int i = 0; i < 512; i++)
  {
    if ((uint64)pagetable == 0x87f70000)
    {
    }
...
int uvmcopy(pagetable_t old, pagetable_t new, pagetable_t newk, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
